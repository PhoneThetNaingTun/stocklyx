...
export const ArchiveCategoryCellAction = ({ data }: Props) => {
  const [open, setOpen] = useState(false);
  const [deleteOpen, setDeleteOpen] = useState(false);
  const [DeleteCategory, { isLoading }] = useDeleteCategoryMutation();
  const [RestoreCategory, { isLoading: RestoreLoading }] =
    useRestoreCategoryMutation();
  const handleDelete = async () => {
    try {
      const responseData = await DeleteCategory({ id: data.id }).unwrap();
      showToast({
  ...
  const handleRestore = async () => {
    try {
      const responseData = await RestoreCategory({ id: data.id }).unwrap();
      showToast({
        title: responseData.message,
        type: "success",
      });
